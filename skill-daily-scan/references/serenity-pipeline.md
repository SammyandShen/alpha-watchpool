...
- `benchmark` 固定 `SPY`
- `sector_benchmark` 按标的行业选最贴近的流动性 ETF：半导体 `SOXX`、软件 `IGV`、生科 `XBI`、能源 `XLE`、工业 `XLI`、金融 `XLF`、非必选消费 `XLY`、通信 `XLC`、公用 `XLU`、医疗 `XLV`、材料 `XLB`、必选消费 `XLP`、房地产 `XLRE`、航天防务 `ITA`、网络安全 `HACK`、云 `SKYY`
